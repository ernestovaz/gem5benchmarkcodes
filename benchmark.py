...
algorithmFilePaths = ['./codes/quicksort', './codes/radixsort', './codes/fft'] #paths to compiled benchmark algorithms.
algNames = ['quicksort', 'radixsort', 'fft']
...
outputFileName = 'numRegs.txt' #name of the file where the IPC and time execution will be written by this program.
...
for index, algPath in algorithmFilePaths:

	algName = algNames[index]
	print("Benchmarking Algorithm: " + algName)
	outputFile = open(outputFilePath, 'a+')
	outputFile.write("\nBenchmarking Algorithm: " + algName + "\n")
	outputFile.close()
	...
	for i in range(numIterations):
		...
		command = './gem5 orgb_configs/simulate.py run-benchmark -c ' + algPath
		os.system(command)
		...
		executionIpc = subprocess.run(['grep', '-oP','issued_per_cycle::mean\\s+\\K\\w+[.]\w+', statsFilePath], 
